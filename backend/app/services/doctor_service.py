...
class DoctorService:
    ...
    def validate_user(self, user_id):
        ...
        if user.role != "Doctor": raise HTTPException(400, "Selected user must have Doctor role")
        return user

    def create(self, data: DoctorCreate):
        if data.user_id is not None: self.validate_user(data.user_id)
        ...
        self.db.add(doctor); self.db.commit(); self.db.refresh(doctor)
        return doctor
    ...
    def update(self, doctor_id, data):
        ...
        if data.user_id is not None: self.validate_user(data.user_id)
        for key, value in data.model_dump().items(): setattr(doctor, key, value)
        self.db.commit(); self.db.refresh(doctor)
        return doctor
