...
def create_appointment(
    ...
    if phone:
        background_tasks.add_task(
            send_appointment_confirmation, phone, a.appointment_number
        )
        background_tasks.add_task(
            send_appointment_reminder, phone, a.appointment_number
        )
    return a
...
def get_appointments(
    ...
    if patient_name:
        q = q.filter(Patient.full_name.ilike(f"%{patient_name}%"))
    if doctor_name:
        q = q.filter(Doctor.full_name.ilike(f"%{doctor_name}%"))
    if appointment_number:
        q = q.filter(Appointment.appointment_number.ilike(f"%{appointment_number}%"))
    if appointment_status:
        q = q.filter(Appointment.status == appointment_status)
    if appointment_date:
...
def update_appointment(
    ...
    return AppointmentService(db).update(
        appointment_id, data, current_user.id, current_user.role
    )
