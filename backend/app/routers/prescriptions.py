...
def create_prescription(
    ...
    phone = (
        db.query(__import__("app.models.patient", fromlist=["Patient"]).Patient.phone)
        .filter(
            __import__("app.models.patient", fromlist=["Patient"]).Patient.id
            == p.patient_id
        )
        .scalar()
    )
    if phone:
        background_tasks.add_task(send_prescription_notification, phone, p.id)
    return p


@router.get("", response_model=list[PrescriptionResponse])
def get_prescriptions(
    db: Session = Depends(get_db), current_user=Depends(require_role("Admin", "Doctor"))
):
...
def update_prescription(
    ...
    if not p:
        raise HTTPException(404, "Prescription not found")
    for k, v in data.model_dump(exclude={"appointment_id"}, exclude_unset=True).items():
        setattr(p, k, v)
    db.commit()
    db.refresh(p)
    return p
