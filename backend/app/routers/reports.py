...
def dashboard(
    ...
    most = (
        db.query(Doctor.full_name, func.count(Appointment.id).label("n"))
    ...
    avg = (
        round(db.query(Appointment).count() / ((today - first).days + 1), 2)
        if first
        else 0
    )
    return {
        ...
        "most_visited_doctor": most.full_name if most else None,
        "average_daily_appointments": avg,
    }
